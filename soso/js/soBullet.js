...
var Actor = function(play){
    //保存剧本的副本
    this.play = play;
    this.color = ['red','green','black','blue','gray','pint','purple']
    this.size = ['18px','20px','22px','16px','24px']
    //给自己化妆
...
Actor.prototype.makeUp = function() {
	var appearance = $('<p>');
	appearance.css({
		fontSize:_random(this.size),
		color:_random(this.color)
	})
	appearance.text(this.play.text);
    return appearance;
}
